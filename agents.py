...
discovery_agent = Agent(
    ...
    model=nebius_model,
    tools=[DuckDuckGoTools(), FirecrawlTools(api_key=os.getenv("FIRECRAWL_API_KEY")), TwitterTools()],
    ...
    instructions=dedent("""
        - Use the `crawl_website` tool on funding platforms like 'https://cryptorank.io/' to discover newly funded utility tokens.
        - For each discovered project URL, use the `scrape_website` tool to extract comprehensive tokenomics, investor profiles, and vesting schedules.
        - Use the `search` tool from DuckDuckGo to find additional articles or analysis to assess the token's utility and competitive landscape.
        - Use the `get_sentiment` tool to analyze Twitter sentiment, filtering out bot activity to gauge authentic bullishness.
        - Cross-reference information from all sources to validate accuracy before presenting the final report.
    """),
